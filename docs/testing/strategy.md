...
- `pwsh -NoProfile -File scripts/package_local.ps1` is the Windows-native source-bundle and gateway-package path; it refuses dirty-tree artifacts by default, auto-resolves a supported Python `>=3.11,<3.14`, refreshes the gateway venv/dist only when gateway packages are built, and writes scope-specific local checksums under `dist/local-release-artifacts/`.
