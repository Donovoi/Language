...
- `pwsh -NoProfile -File scripts/package_local.ps1 -Python <path-to-supported-python>` is the Windows-native source-bundle and gateway-package path; it refuses dirty-tree artifacts by default, refreshes the gateway venv/dist only when gateway packages are built, and writes scope-specific local checksums under `dist/local-release-artifacts/`.
