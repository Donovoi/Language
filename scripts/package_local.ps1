...
function Get-PythonSupportError {
    ...
    if ($version.Major -ne 3 -or $version.Minor -lt 11 -or $version.Minor -ge 14) {
        return "$Label uses Python $version, but the gateway packaging path supports >=3.11,<3.14."
    }
    return ""
}
...
function Get-PythonCandidates {
    ...
    if (-not [string]::IsNullOrWhiteSpace($env:LANGUAGE_PACKAGE_PYTHON)) {
        $candidates.Add([pscustomobject]@{ Label = "LANGUAGE_PACKAGE_PYTHON"; Candidate = $env:LANGUAGE_PACKAGE_PYTHON })
    }
...
function Resolve-SupportedPython {
    ...
    $details = $errors -join " "
    throw "Could not find a supported Python >=3.11,<3.14 for gateway packaging. Pass -Python or set LANGUAGE_PACKAGE_PYTHON. $details"
}
...
function Build-GatewayPackage {
    ...
    Invoke-CheckedCommand `
        ...
        -WorkingDirectory $gatewayDir
    $gatewayPackageVerifier = Join-Path $RepoRoot "scripts\verify_gateway_package.py"
    Invoke-CheckedCommand `
        -Label "verify gateway console entry point" `
        -FilePath $gatewayPython `
        -Arguments @($gatewayPackageVerifier, "--installed") `
        -WorkingDirectory $gatewayDir
    Invoke-CheckedCommand `
    ...
    Invoke-CheckedCommand `
        ...
        -FilePath $gatewayPython `
        -Arguments @($gatewayPackageVerifier, "--wheel", $wheels[0].FullName, "--sdist", $sdists[0].FullName) `
        -WorkingDirectory $gatewayDir
