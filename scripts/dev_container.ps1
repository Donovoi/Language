...
switch ($Action) {
    ...
    "test-category" {
        $testCategoryArgs = $CommandArgs
        & python scripts/run_test_category.py @testCategoryArgs
        if ($LASTEXITCODE -ne 0) {
            exit $LASTEXITCODE
        }
    }
    ...
    "headphone-isolation-preflight" {
        $preflightArgs = $CommandArgs
        & python scripts/run_headphone_isolation_check.py preflight @preflightArgs
        if ($LASTEXITCODE -ne 0) {
            exit $LASTEXITCODE
        }
    }
    ...
    "headphone-isolation-collect-evidence" {
        $collectArgs = $CommandArgs
        & python scripts/run_headphone_isolation_check.py collect-headphone-evidence @collectArgs
        if ($LASTEXITCODE -ne 0) {
            exit $LASTEXITCODE
        }
    }
    ...
    "audio-eval-same-voice-candidate-contract-check" {
        Invoke-Compose --profile audio-eval build audio-eval
        Invoke-Compose --profile audio-eval run --rm audio-eval python3 scripts/benchmark_same_voice_candidate_fixture.py --self-test
    }
    "audio-eval-same-voice-candidate-check" {
        $sameVoiceCandidateArgs = $CommandArgs
        Invoke-Compose --profile audio-eval build audio-eval
        Invoke-Compose --profile audio-eval run --rm audio-eval python3 scripts/benchmark_same_voice_candidate_fixture.py check @sameVoiceCandidateArgs
    }
    ...
    "audio-eval-speechbrain-voice-similarity-contract-check" {
        Invoke-Compose --profile audio-eval build audio-eval
        Invoke-Compose --profile audio-eval run --rm audio-eval python3 scripts/run_speechbrain_voice_similarity_fixture.py --self-test
    }
    "audio-eval-speechbrain-voice-similarity-check" {
        $speechBrainVoiceSimilarityArgs = $CommandArgs
        Invoke-Compose --profile audio-eval-speechbrain-sepformer build audio-eval-speechbrain-sepformer
        Invoke-Compose --profile audio-eval-speechbrain-sepformer run --rm audio-eval-speechbrain-sepformer python3 scripts/run_speechbrain_voice_similarity_fixture.py score @speechBrainVoiceSimilarityArgs
    }
